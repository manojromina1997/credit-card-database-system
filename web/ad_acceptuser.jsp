      ...
      <div class="col-md-3 left_col">
        <div class="left_col scroll-view">
          ...
          <div class="navbar nav_title" style="border: 0;">
              <a href="ad_manage.jsp" class="site_title"><span>National Bank</span></a>
          </div>
...
<div class="right_col" role="main">
        <div class="">
          ...
          <div class="row">
            ...
            PreparedStatement pst=conn.prepareStatement("SELECT * FROM Newcustomer where firstname='"+request.getParameter("id")+"'");
 ...
 <%
        ...
        %>
        <h4 style="color:blue">
               <form action="Register_customer" method="post" style="font-size:20px;text-align:left">
                  First Name: <%=resultset.getString(8)%><br>
                   <input type="hidden" name="firstname" value="<%=resultset.getString(8)%>"/> 
                  Last Name: <%=resultset.getString(9)%><br>
                   <input type="hidden" name="lastname" value="<%=resultset.getString(9)%>"/>  
                   Dob: <%=resultset.getString(6)%><br>
                   <input type="hidden" name="dob" value="<%=resultset.getString(6)%>"/>
                    City: <%=resultset.getString(2)%><br>
                   <input type="hidden" name="city" value="<%=resultset.getString(2)%>"/> 
                  Address: <%=resultset.getString(4)%><br>
                   <input type="hidden" name="address" value="<%=resultset.getString(4)%>"/>
                    Occupation: <%=resultset.getString(11)%><br>
                   <input type="hidden" name="occupation" value="<%=resultset.getString(11)%>"/> 
                  Qualification: <%=resultset.getString(12)%><br>
                   <input type="hidden" name="qualification" value="<%=resultset.getString(12)%>"/>  
                  Annual Income: <%=resultset.getString(3)%><br>
                   <input type="hidden" name="annualincome" value="<%=resultset.getString(3)%>"/>
                   Creditcard: <%=resultset.getString(5)%><br>
                   <input type="hidden" name="creditcard" value="<%=resultset.getString(5)%>"/>
                    Email: <%=resultset.getString(7)%><br>
                   <input type="hidden" name="emailid" value="<%=resultset.getString(7)%>"/> 
                  Mobile: <%=resultset.getString(10)%><br>
                   <input type="hidden" name="mobile" value="<%=resultset.getString(10)%>"/>  
                    Username:
                    <input type="text" name="username" placeholder="username"/>  <br>
                    Password
                    <input type="password" name="password" placeholder="password"/> <br> 
          </h4>
                    <a href="Delete_newcustomer"> <button type="submit" class="btn btn-success">Accept</button></a>
               </form>
